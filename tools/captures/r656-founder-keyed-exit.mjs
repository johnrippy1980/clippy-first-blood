// R656: FOUNDER'S LAIR is the first campaign stage to USE the R655 EXIT KEY
// lock. makeStage6() sets `exitKey:true` and hides an EXITKEY pickup on the
// FORK-3 high catwalk (col 50, row 3 — one tile above the row-4 platform).
// This probe proves the stage is wired end-to-end on the REAL campaign
// geometry (not a runtime-injected flag like r655):
//
//   - stage 11 really is FOUNDER (so we're gating the intended stage),
...
check('stage 11 is FOUNDER\'S LAIR', authored.theme === 'founder', JSON.stringify(authored));
check('FOUNDER\'S LAIR authors exitKey:true', authored.exitKey);
...
check('EXIT KEY rests one tile above the FORK-3 catwalk', authored.tileBelowKey === 2, 'tileBelowKey=' + authored.tileBelowKey);

// --- Reach the PLAY scene on stage 11 so the exit-check actually runs. ---
await page.evaluate(async () => {
  const g = window.__game;
  g._startStage(11);
  await new Promise(r => setTimeout(r, 120));
  const lvl = g.level;
  // disarm the boss TRIGGER so reaching the exit tile doesn't spawn the founder
  // mid-test; we drive boss-DEFEAT state directly below.
...
const lockedWithBoss = await page.evaluate(() => {
  ...
  // mark the boss as already defeated so ONLY the key-gate is under test.
...
check('on the EXIT tile in PLAY scene (stage 11)', lockedWithBoss.scene === 'play' && lockedWithBoss.isExit === true, JSON.stringify(lockedWithBoss));
...
check('keyed + boss-down clears FOUNDER\'S LAIR', cleared.cleared === true, JSON.stringify(cleared));

check('no page errors', errs.length === 0, errs.slice(0, 2).join(' | '));

await browser.close();
console.log(fails === 0 ? 'R656 PASS' : ('R656 FAIL (' + fails + ')'));
