...
await fs.writeFile('/tmp/r563_jab.png', Buffer.from(u1.replace(/^data:image\/png;base64,/, ''), 'base64'));
// Second — CROSS
await page.keyboard.press('KeyX');
await page.waitForTimeout(150);
const u2 = await page.evaluate(() => document.getElementById('screen')?.toDataURL('image/png'));
await fs.writeFile('/tmp/r563_cross.png', Buffer.from(u2.replace(/^data:image\/png;base64,/, ''), 'base64'));
// Third — HOOK
await page.keyboard.press('KeyX');
await page.waitForTimeout(150);
const u3 = await page.evaluate(() => document.getElementById('screen')?.toDataURL('image/png'));
await fs.writeFile('/tmp/r563_hook.png', Buffer.from(u3.replace(/^data:image\/png;base64,/, ''), 'base64'));
await browser.close();
